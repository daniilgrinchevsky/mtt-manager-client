...
import {getColorFromPriority} from "../../Common/utils/common";
            ...
            <div className={divclassName}>
                <table className={tableclassName}>
                    ...
                    {getRowModel().rows.map(row => {
                        return (
                            <tr key={row.id} onClick={(event) => rowClickHandler(row.original)} className={trclassName}>
                                {row.getVisibleCells().map(cell => {
                                    return (
                                        <td key={cell.id}
                                            className={hasPriority ? tdclassName + " text-" + getColorFromPriority(row.original.priority) + "-500" : tdclassName}
                                            >
                                            {flexRender(
